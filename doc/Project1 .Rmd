...
presid.summary[,2] <- gsub(NA, 0, presid.summary[,2])
for(i in 2:9){
...
get_query("Abraham Lincoln", 1, "sqr")


get_query("George W. Bush", 2, "sqr")
get_query("George W. Bush", 2, "cos")

get_query("George Bush", 1, "sqr")
```

As the examples and the clustering plots inthe previous part of this notebook, we can find out that the speeches that close to each other are always happend on similiar situation of america, no matter in economics, freedom, equality and so on. So next time when a new speech comes, we can get to know the appropriate situation that America is at by analyzing the topics or the tf-idf of the words.
